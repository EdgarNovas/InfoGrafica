...
void main(){
	...
	if (glewInit() == GLEW_OK) {
		...
		while (!glfwWindowShouldClose(window)) {
			...
			//Calculamos nueva opsicion
			cube.position = cube.position + cube.forward * cube.velocity;
			cube.rotation = cube.rotation + glm::vec3(0.f, 1.f, 0.f) * cube.angularVelocity;

			//Invertimos direccion si sale de limites
			...
			//Genero matrix de translacion
			glm::mat4 cubeTranslationMatrix = GenerateTranslationMatrix(cube.position);
			//Generar matriz de rotacion
			glm::mat4 cuberotacionMatrix = GenerateRotationMatrix(glm::vec3(0.f, 1.f, 0.f), cube.rotation.y);

			//Aplical Matrix
			cubeModelMatrix = cubeTranslationMatrix* cuberotacionMatrix * cubeModelMatrix;
